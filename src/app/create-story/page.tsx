...
export default function CreateStoryPage() {
  ...
  const handlePublish = async () => {
    if (!user || !mediaData || !profile) return;
    ...
    try {
      ...
      const uploadedUrl = await uploadToCloudinary(mediaData, mediaType);
      ...
      toast({ title: isRtl ? "تم نشر القصة" : "Story Published" });
      router.push("/");
    } catch (e) {
      toast({ variant: "destructive", title: "فشل الرفع والنشر" });
    } finally {
  ...
  return (
    <div className="min-h-screen bg-black text-white max-w-md mx-auto relative flex flex-col pb-10">
      <header className="p-6 flex items-center justify-between border-b border-zinc-900 bg-black/80 backdrop-blur-xl sticky top-0 z-20">
        ...
        <Button onClick={handlePublish} disabled={!mediaData || isUploading || (profile?.points < 50)} className="rounded-full px-8 bg-white text-black font-black">
          {isUploading ? <Loader2 className="h-4 w-4 animate-spin" /> : (isRtl ? "نشر" : "Share")}
        </Button>
      </header>
          ...
          {mediaData ? (
            <div className="w-full h-full">
              {mediaType === "image" ? (
                ...
                <video src={mediaData} className="w-full h-full object-cover" autoPlay muted loop />
              )}
            </div>
          ) : (
            <div className="flex flex-col items-center gap-4">
              <Camera className="h-12 w-12 text-zinc-700" />
              <p className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">{isRtl ? "اختر صورة أو فيديو" : "Select Image or Video"}</p>
            </div>
          )}
        </div>

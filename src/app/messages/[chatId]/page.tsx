...
export default function DirectChatRoomPage() {
  ...
  const handleSend = () => {
    ...
    addDoc(collection(db, "direct_conversations", chatId as string, "messages"), {
      senderId: user.uid,
      text: newMessage,
    ...
    <div className="flex flex-col h-screen bg-black text-white max-w-md mx-auto relative shadow-2xl border-x border-zinc-900 overflow-hidden">
      <header className="p-4 border-b border-zinc-900 bg-black/80 backdrop-blur-md flex items-center justify-between sticky top-0 z-20">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" className="rounded-full" onClick={() => router.back()}><ArrowLeft className={cn("h-6 w-6", isRtl ? "rotate-180" : "")} /></Button>
          <div className="flex items-center gap-3">
        ...
        <Button variant="ghost" size="icon" className="rounded-full text-zinc-400"><MoreVertical className="h-5 w-5" /></Button>
      </header>
      ...
      <footer className="p-4 border-t border-zinc-900 bg-black">
        {!isFriend ? (
          <div className="p-4 bg-red-500/10 rounded-[2rem] border border-red-500/20 text-center space-y-2">
             <div className="flex justify-center mb-1"><Lock className="h-4 w-4 text-red-500" /></div>
             <p className="text-[10px] font-black text-red-500 uppercase tracking-widest">{isRtl ? "المراسلة للأصدقاء فقط" : "Messages restricted to friends"}</p>
          </div>
        ) : (
